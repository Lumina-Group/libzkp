...
pub fn prove_equality(val1: u64, val2: u64) -> PyResult<Vec<u8>> {
    ...
    let commitment_arr: [u8; 32] = match commitment.clone().try_into() {
        Ok(arr) => arr,
        Err(_) => return Err(PyErr::from(ZkpError::InvalidProofFormat("invalid commitment size".to_string()))),
    };
...
pub fn verify_equality(proof: Vec<u8>, val1: u64, val2: u64) -> PyResult<bool> {
    ...
    if val1 != val2 { return Ok(false); }

    let expected_commitment = commit_value(val1);
    if validate_standard_commitment(&expected_commitment).is_err() { return Ok(false); }

    if proof.commitment != expected_commitment { return Ok(false); }
